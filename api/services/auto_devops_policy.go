...
// parseEnvDefault reads DD_UI_DEVOPS_APPLY: "true"/"false" -> bool; anything else => unset (nil).
func parseEnvDefault() *bool {
	raw := strings.TrimSpace(os.Getenv("DD_UI_DEVOPS_APPLY"))
	if raw == "" {
		return nil
	}
...
func effectiveAutoDevops(ctx context.Context, stackID int64) (bool, string) {
	...
	var groups []string
	if scopeKind == "host" {
		h, err := database.GetHostByName(ctx, scopeName)

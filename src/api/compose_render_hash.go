...
func renderComposeServices(ctx context.Context, stageDir, projectName string, files []string) ([]RenderedService, error) {
	...
	for _, entry := range entries {
		...
		if strings.HasSuffix(entry.Name(), ".env") {
			...
			if entry.Name() == ".env" {
				rootEnv = envVars
				debugLog("Parsed root .env file with %d variables (already decrypted in staging)", len(envVars))
				for k, v := range envVars {
					debugLog("  Root .env: %s = %s", k, v)
				}
			} else {
				debugLog("Parsed env file %s with %d variables (already decrypted in staging)", entry.Name(), len(envVars))
				for k, v := range envVars {
					debugLog("  %s: %s = %s", entry.Name(), k, v)
				}
			}
			...
			if err != nil {
				debugLog("Failed to read compose file %s: %v", entry.Name(), err)
				continue // Skip problematic compose files
			}
			debugLog("Processed compose file %s (already decrypted in staging)", entry.Name())
			...
			content, err := os.ReadFile(sourcePath)
			if err != nil {
				debugLog("Failed to read file %s: %v", entry.Name(), err)
				continue
			}
			destPath := filepath.Join(tempDir, entry.Name())
			if err := os.WriteFile(destPath, content, 0644); err != nil {
	...
	debugLog("Running docker compose config in %s with files: %v", tempDir, composeFiles)
	cmd := exec.CommandContext(ctx, "docker", args...)
	cmd.Dir = tempDir
	out, err := cmd.CombinedOutput()
	...
	if err == nil {
		...
		if err := json.Unmarshal(out, &payload); err == nil {
			...
			for name, sv := range payload.Services {
				...
				debugLog("Docker rendered service %s: image=%s, container=%s", name, sv.Image, sv.ContainerName)
			}
	...
	debugLog("Docker compose config failed or parsing failed: %v, using manual parsing", err)
...
func parseComposeWithVariableResolution(ctx context.Context, tempDir string, composeFiles []string, rootEnv map[string]string, envFiles map[string]map[string]string) ([]RenderedService, error) {
	debugLog("Using manual compose parsing with variable resolution")
	...
	for _, filename := range composeFiles {
		...
		if err != nil {
			debugLog("Failed to read compose file %s: %v", filename, err)
			continue
		}
		...
		if err := json.Unmarshal(content, &compose); err != nil {
			...
			if err := yaml.Unmarshal(content, &compose); err != nil {
				debugLog("Failed to parse compose file %s as JSON or YAML: %v", filename, err)
				continue
			}
			debugLog("Successfully parsed %s as YAML", filename)
		} else {
			debugLog("Successfully parsed %s as JSON", filename)
		}
		...
		for serviceName, service := range compose.Services {
			...
			debugLog("Resolving variables for service %s", serviceName)
			debugLog("  Original image: %s", service.Image)
			debugLog("  Original container_name: %s", service.ContainerName)
			...
			debugLog("  Resolved image: %s", image)
			debugLog("  Resolved container_name: %s", containerName)
			...
			debugLog("Manual parsed service %s: image=%s, container=%s", serviceName, image, containerName)
		}
	}
...
func resolveVariablesWithPrecedence(input string, rootEnv, serviceEnv map[string]string, serviceEnvFiles []map[string]string) string {
	...
	for {
		...
		if source != "" {
			debugLog("Resolved %s=%s from %s", varName, value, source)
		} else if value == "" {
			debugLog("Variable %s not found in any source, leaving unresolved", varName)
		}

...
func (p *SSHConnectionPool) getSSHConnection(user, host, keyFile string) (*ssh.Client, error) {
	...
	if conn, exists := p.connections[key]; exists {
		...
		if conn.client != nil {
			...
			if err == nil {
				...
				conn.mu.RUnlock()
				debugLog("SSH: Reusing existing connection to %s", key)
	...
	debugLog("SSH: Creating new connection to %s", key)
...
func createSSHClient(user, host, keyFile string) (*ssh.Client, error) {
	...
	debugLog("SSH: Successfully connected to %s@%s", user, host)
	return client, nil
}
...
func createSSHDockerClient(user, host, keyFile string) (*client.Client, func(), error) {
	...
	debugLog("SSH: Created Docker client with SSH transport for %s@%s", user, host)

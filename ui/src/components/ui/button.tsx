...
export function Button({ className="", variant="default", size="md", ...rest }: Props) {
  ...
  const v =
    variant === "outline" ? "border border-slate-700 text-slate-200 bg-transparent hover:bg-slate-800" : variant === "ghost" ? "bg-transparent hover:bg-slate-900" : "bg-slate-800 hover:bg-slate-700 text-white";
  const s =
    size === "sm" ? "h-8 px-3 text-sm" : size === "icon" ? "h-10 w-10" : "h-10 px-4";
